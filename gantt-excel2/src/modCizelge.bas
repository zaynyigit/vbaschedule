...
Public Sub Ciz()
    ...
    ' Ayin tatil gunleri bir kez okunur, asagida hep bu diziden bakilir
    modVeri.OffGunleriOku yil, ay, tatil

    ws.Unprotect
    EskiSekilleriSil ws
    GovdeyiTemizle ws
    TakvimBasliginiCiz ws, yil, ay, gunSayisi, tatil
    SatirlariCiz ws, yil, ay, gunSayisi, tatil
...
' Cizelge bir gorunum, tablo degil. Kullanici barin uzerine yazi yazip
' cizimi bozmasin diye sayfa kilitlenir; kodun kendisi UserInterfaceOnly
' sayesinde yazmaya devam edebilir. Cift tiklama ile gorev ekleme, kod
' icinde Locked = False yapilan gun hucreleri uzerinden calisir.
Private Sub SayfayiKilitle(ByVal ws As Worksheet)
    On Error Resume Next
    ws.Protect UserInterfaceOnly:=True, DrawingObjects:=False, _
...
Private Sub GovdeyiTemizle(ByVal ws As Worksheet)
    ...
    With ws.Range(ws.Cells(R_AYADI, 1), ws.Cells(sonSatir + 60, C_GUN1 + GUN_SAYISI_MAX + 1))
        .ClearContents
        .ClearComments          ' ClearContents aciklamayi silmez; sorumlu
                                ' balonlari eski satirlarda hayalet kaliyordu
        .Interior.Color = CLR_ZEMIN()
        .Borders.LineStyle = xlNone
        .UnMerge
        .Locked = True          ' sadece gun hucreleri sonra aciliyor
    End With
...
Private Sub SatirlariCiz(ByVal ws As Worksheet, ByVal yil As Long, _
    ...
    Dim gorevler() As TGorev, adet As Long
    Dim projeAdet As Long
    Dim toplamYuzde As Double
    Dim ilkGun As Long, sonGun As Long, pIlk As Long, pSon As Long

    Set wsP = modVeri.Sayfa(SH_PROJELER)
    modVeri.AyinGorevleriniOku yil, ay, gorevler, adet
    satir = R_VERI_BAS

    For i = 2 To modVeri.SonSatir(wsP)
        projeID = CLng(Val(wsP.Cells(i, P_ID).Value))
        If projeID > 0 Then
            ' --- projenin bu aya dusen gorevlerini olc ---
            projeAdet = 0: toplamYuzde = 0: pIlk = GUN_SAYISI_MAX + 1: pSon = 0
            For j = 1 To adet
                If gorevler(j).ProjeID = projeID Then
                    projeAdet = projeAdet + 1
                    toplamYuzde = toplamYuzde + gorevler(j).Yuzde
                    AyIcindekiAralik gorevler(j), yil, ay, gunSayisi, ilkGun, sonGun
                    If ilkGun > 0 Then
            ...
            If projeAdet > 0 Then
                ...
                If p.ID = 0 Then p.Ad = CStr(wsP.Cells(i, P_AD).Value)

                ProjeSatiriCiz ws, satir, p, projeAdet, gunSayisi, pIlk, pSon, _
                               toplamYuzde / projeAdet
                satir = satir + 1

                ' --- gorev satirlari (Sira'ya gore sirali gelir) ---
                For j = 1 To adet
                    If gorevler(j).ProjeID = projeID Then
                        GorevSatiriCiz ws, satir, gorevler(j), p, yil, ay, gunSayisi, tatil
                        satir = satir + 1
                    End If
                Next j
            End If
...
Private Sub GorevSatiriCiz(ByVal ws As Worksheet, ByVal satir As Long, ByRef g As TGorev, _
                           ...
                           ByVal gunSayisi As Long, ByRef tatil() As Boolean)
...
Private Sub BarCiz(ByVal ws As Worksheet, ByVal satir As Long, ByVal ilkGun As Long, _
    ...
    ' Yorum rozeti - sayi cizim basinda tek gecisle dolduruldu
    If g.YorumAdedi > 0 Then RozetCiz ws, g.ID, sol + gen - 9, ust - 4, g.YorumAdedi
End Sub
