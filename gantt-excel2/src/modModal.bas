...
Private Const C_LISTE     As Long = 8      ' gizli sutun: uzun acilir liste kaynagi
...
Private mListeSatir As Long        ' gizli liste sutununda siradaki bos satir
...
Public Sub Goster(ByVal amac As String, ByVal baslik As String, _
    ...
    ws.Columns(C_LISTE).Hidden = True
    mListeSatir = 1
    ...
    For i = 1 To mSayi
        ...
        Else
            ...
            ' Giris satiri
            ws.Rows(r).RowHeight = AlanYuksekligi(i)
...
' Satir yuksekligi. Salt-okunur alanlar cok satirli metin tasiyor (yorum
' gecmisi, kullanici listesi); sabit 22 puntoda metnin tamami gorunmuyordu.
' Satir sayisina gore buyuyor, panelden tasmamasi icin ust siniri var.
Private Function AlanYuksekligi(ByVal alanNo As Long) As Double
    Dim n As Long, h As Double
    Select Case mAlanTip(alanNo)
        Case MT_COKSATIR
            AlanYuksekligi = 52
        Case MT_SALTOKUR
            n = SatirSayisi(mAlanDeger(alanNo))
            h = 6 + n * 13.5
            If h < 22 Then h = 22
            If h > 190 Then h = 190
            AlanYuksekligi = h
...
Private Function SatirSayisi(ByVal metin As String) As Long
    Dim n As String
    n = Replace(metin, vbCrLf, vbLf)
    SatirSayisi = Len(n) - Len(Replace(n, vbLf, "")) + 1
End Function
...
Private Sub GirisHucresiKur(ByVal hc As Range, ByVal alanNo As Long)
    With hc
        ...
        .VerticalAlignment = IIf(mAlanTip(alanNo) = MT_SALTOKUR, xlTop, xlCenter)
        .IndentLevel = 1
        .WrapText = (mAlanTip(alanNo) = MT_COKSATIR Or mAlanTip(alanNo) = MT_SALTOKUR)
    ...
    Select Case mAlanTip(alanNo)
        ...
        Case MT_LISTE, MT_ONAY
            ...
            DogrulamaKur hc, liste
    End Select
End Sub

' Excel'in dogrudan yazilan dogrulama listesi 255 karakterle sinirli. Proje ya
' da kullanici listesi bunu asinca eskiden acilir liste sessizce hic olusmuyordu.
' Uzun listeler gizli bir sutuna yazilip aralik olarak baglaniyor.
Private Sub DogrulamaKur(ByVal hc As Range, ByVal liste As String)
    Dim ws As Worksheet, p As Variant, i As Long, ilk As Long, kaynak As String
    ...
    If Len(liste) <= 250 Then
        kaynak = liste
    Else
        p = Split(liste, ",")
        ilk = mListeSatir
        For i = LBound(p) To UBound(p)
            ws.Cells(mListeSatir, C_LISTE).Value = Trim$(p(i))
            mListeSatir = mListeSatir + 1
        Next i
        kaynak = "=" & ws.Range(ws.Cells(ilk, C_LISTE), _
                                ws.Cells(mListeSatir - 1, C_LISTE)).Address(True, True)
    End If

    On Error Resume Next
    hc.Validation.Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, _
                      Operator:=xlBetween, Formula1:=kaynak
...
Public Sub Kapat()
    Dim ws As Worksheet, hedef As Worksheet
    On Error Resume Next
    Set ws = PanelSayfasi()

    ' Excel etkin sayfayi gizlemeye izin vermez. Once cizelgeye gecip sonra
    ' paneli gizlemek gerekiyor; ters sirada panel sekmesi ekranda kaliyordu.
    Set hedef = modVeri.Sayfa(SH_CIZELGE)
    If Not hedef Is Nothing Then
        If hedef.Visible <> xlSheetVisible Then hedef.Visible = xlSheetVisible
        hedef.Activate
    End If
