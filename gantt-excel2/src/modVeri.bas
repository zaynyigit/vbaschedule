...
Public Type TGorev
    ...
    Aciklama        As String
    SatirNo         As Long
    YorumAdedi      As Long        ' cizim sirasinda tek okumada doldurulur
End Type
...
' Hucredeki degeri tarihe cevirir. Bos ya da bozuk deger 0 doner.
' CDate bos hucrede "Tip uyusmazligi" verir; tek bir bozuk satir yuzunden
' butun cizimin durmamasi icin her tarih okumasi buradan geciyor.
Public Function TarihOku(ByVal v As Variant) As Date
    If IsDate(v) Then TarihOku = CDate(v)
End Function
...
Public Function GorevOku(ByVal satir As Long) As TGorev
    ...
    g.PlanBaslangic = TarihOku(ws.Cells(satir, G_PLANBAS).Value)
    g.PlanBitis = TarihOku(ws.Cells(satir, G_PLANBIT).Value)
    g.GerceklesenBitis = CDbl(TarihOku(ws.Cells(satir, G_GERCEK).Value))
...
' Verilen ayla kesisen gorevleri TEK okumada diziye alir ve Sira'ya gore siralar.
...
'
' Eskiden cizim kodu her proje icin butun gorevleri tekrar okuyordu; 7 gorevde
' fark edilmiyor ama 200 gorevde sayfa okumasi kareleniyordu. Artik gorevler
' bir kez okunur, yorum sayilari da tek gecisle doldurulur.
Public Sub AyinGorevleriniOku(ByVal yil As Long, ByVal ay As Long, _
                              ByRef gorevler() As TGorev, ByRef adet As Long)
    Dim ws As Worksheet, i As Long, j As Long, g As TGorev, tmp As TGorev
    ...
    adet = 0
    ReDim gorevler(1 To 1)
    If son < 2 Then Exit Sub
    ReDim gorevler(1 To son)

    For i = 2 To son
        If ws.Cells(i, G_ID).Value <> "" Then
            g = GorevOku(i)
            ' Tarihi bozuk satiri sessizce atla - cizim durmasin
            If g.PlanBaslangic > 0 And g.PlanBitis > 0 Then
    ...
    ' Yorum sayilari - _Yorumlar sayfasinda tek gecis
    Dim wsY As Worksheet, gid As Long
    Set wsY = Sayfa(SH_YORUMLAR)
    For i = 2 To SonSatir(wsY)
        gid = CLng(Val(wsY.Cells(i, Y_GOREV).Value))
        If gid > 0 Then
            For j = 1 To adet
                If gorevler(j).ID = gid Then
                    gorevler(j).YorumAdedi = gorevler(j).YorumAdedi + 1
                    Exit For
                End If
            Next j
        End If
    ...
    ' Sira'ya gore sirala (ekleme siralamasi - kucuk listelerde en okunur olan)
    For i = 2 To adet
        tmp = gorevler(i)
        j = i - 1
        Do While j >= 1
            If gorevler(j).Sira <= tmp.Sira Then Exit Do
            gorevler(j + 1) = gorevler(j)
            j = j - 1
        Loop
        gorevler(j + 1) = tmp
    Next i
...
Public Sub GorevSil(ByVal gorevID As Long)
    ...
    GecikmeSil gorevID          ' yoksa oksuz gecikme kaydi kaliyordu
End Sub
...
Public Sub GecikmeSil(ByVal gorevID As Long)
    Dim ws As Worksheet, i As Long
    Set ws = Sayfa(SH_GECIKMELER)
    For i = SonSatir(ws) To 2 Step -1
        If CLng(Val(ws.Cells(i, D_GOREV).Value)) = gorevID Then ws.Rows(i).Delete
    Next i
End Sub
...
Public Function OffGunMu(ByVal tarih As Date) As Boolean
    ...
    For i = 2 To SonSatir(ws)
        If TarihOku(ws.Cells(i, O_TARIH).Value) = tarih Then
            OffGunMu = True
            Exit Function
        End If
    Next i
...
' Ayin tatil gunlerini tek okumada bayrak dizisine cevirir.
' Cizim her gun hucresi icin OffGunMu cagirdiginda _OffGunler sayfasi
' yuzlerce kez tarandigi icin eklendi: 1 tarama, 31 bayrak.
Public Sub OffGunleriOku(ByVal yil As Long, ByVal ay As Long, ByRef bayrak() As Boolean)
    Dim ws As Worksheet, i As Long, t As Date
    ReDim bayrak(1 To GUN_SAYISI_MAX)
    Set ws = Sayfa(SH_OFFGUNLER)
    For i = 2 To SonSatir(ws)
        t = TarihOku(ws.Cells(i, O_TARIH).Value)
        If t > 0 Then
            If Year(t) = yil And Month(t) = ay Then bayrak(Day(t)) = True
        End If
    Next i
End Sub
